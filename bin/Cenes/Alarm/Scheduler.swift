...
class Scheduler {
    ...
    func setNotificationWithDate(_ date: Date, weekdays: [Int], sound: String, alarmName: String, identifier: String){
        ...
        alarmNotification.soundName = sound
        alarmNotification.timeZone = .current */
        
//        let repeating: Bool = !weekdays.isEmpty
//        alarmNotification.userInfo = ["sound": sound, "repeating": repeating]
//
//        if !weekdays.isEmpty {
//            alarmNotification.repeatInterval = NSCalendar.Unit.weekOfYear
//        }
        
        let datesForNotification = correctDate(date, onWeekdaysForNotify: weekdays, sound: sound)
        
        for d in datesForNotification {
            setupNotification(date: d, alarmName: alarmName, identifier: identifier)
        }
    }
    
    func setupNotification(date: Date, alarmName: String, identifier: String){
        ...
        let newComponents = DateComponents(calendar: calendar, timeZone: .current, month: currentDateComponents.month, day: currentDateComponents.day, hour: components.hour, minute: components.minute, second: components.second)
