...
class GatheringTableViewCellFive: UITableViewCell,CreateGatheringViewControllerDelegate ,NVActivityIndicatorViewable{
    ...
    let commonOrange = UIColor(red: 255/255, green: 43/255, blue: 21/255, alpha: 0.9)
    ...
    func setUpdatedDate(){
        ...
        if self.gatheringView.FirstDate >= self.gatheringView.SecondDate{
            ...
            var datecomponent = DateComponents()
            datecomponent.minute = 15
            let startDate = Calendar.current.date(byAdding: datecomponent, to: self.gatheringView.FirstDate)
    ...
    func setFifthCase(){
        ...
        self.switchPredictive.isOn = self.gatheringView.isPreditiveEnabled
        ...
        if self.gatheringView.isPreditiveEnabled {
    ...
    @IBAction func switchChanged(_ sender: UISwitch) {
        ...
        if sender.isOn {
            self.gatheringView.isPreditiveEnabled = true
            self.getPreditveData()
            
        }else{
            self.gatheringView.isPreditiveEnabled = false
...
//        print("milliseconds End =\(endDateNew?.millisecondsSince1970)")
        self.gatheringView.startAnimating(loadinIndicatorSize, message: "Loading...", type: NVActivityIndicatorType(rawValue: 15))
...
//        }
    ...
    extension GatheringTableViewCellFive: FSCalendarDelegate , FSCalendarDelegateAppearance ,FSCalendarDataSource
    {
